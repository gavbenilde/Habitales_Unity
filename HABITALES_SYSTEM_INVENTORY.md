...
| **Neglect decay** (added 2026-06-25) | `Tile.cs` (`decayK`, `DecayStart`=0.1, `DecayGrowth`=1.01, `ResetDecay()`), `TileManager.ApplyDailyDecay()` + `ResetTileDecay()`, `RunManager` heartbeat step 2b, `ActionManager.FinishAction` | — | ACTIVE | Per-tile exponentially escalating decay. Each resolved day every tile loses `decayK` from its 6 soil substats + vegetation cover (clamped ≥0), then `decayK *= DecayGrowth`. Working a tile (`ExecuteOnTile`) resets `decayK` to `DecayStart`. Contamination + derived SoilComposite untouched. Decay runs before cascade so the loss diffuses. |
...
> 🗂️ **Targeted update 2026-06-27 — Action UI decomposition (ActionBarUI → controller + passive views; legacy ActionUI deleted).** The monolithic action-bar UI was split following the `ACTIONUI_REFACTOR_PLAN.md` (repo root). **Communication model:** a **Controller ↔ View contract** in new `namespace Habitales.UI.Actions` (all files stay in `_GAME/Action/` — no folder move) — downward = `[SerializeField]` method calls + Law-3 loud-fail; upward = C# meaning-events (Law 2); views never write `TileSelector`/`ActionManager` (Law 1, controller mediates). **`ActionBarUI` kept its class name + script GUID** (scene/RunManager/Onboarding refs survive) and is now the **controller only** (state machine + cross-system + the frozen onboarding surface: `OnActionArmed`/`OnActionConfirmed`/`CurrentArmedAction`/`GetArmCueRect`/`GetConfirmButtonRect`). Five new view scripts (below). **`ActionUI` (907) DELETED** (Open Question #2 resolved); `InspectModeManager` no longer depends on it (owns its own `_inspectMode` bool); `RunManager`'s dormant `actionUI` field removed. **Orphan-cleanup pass (2026-06-27, verified by GUID search across all of `Assets/` incl. `.unity`/`.prefab`):** `HoverTooltip.cs` (`_GAME/UI/HoverToolttip.cs`, GUID `3e64723616cf039429971f8081a343fd`) had **zero** references in code OR assets → **REMOVED 2026-06-27 (deleted with its `.meta`).** **Second orphan-cleanup pass (2026-06-27, GUID + code-ref verified):** `ActionPanelState.cs` (GUID `2bd4074276c86c840bf2c3a2f8492a55`) had zero GUID hits across all assets and zero non-comment code references → **REMOVED 2026-06-27**. `ActionIconConfig.cs` (GUID `fd887aa856de3d6448b7b55fcf9a26d3`) had zero live asset refs (its `.asset` GUID `cd0e5dff4ca9f434ca03c2d83ead90ae` only appears in scratch scenes `casdasda.unity`/`0.unity`, NOT `Vertical Slice.unity`); its only appearance in `PlayerAction.cs` is inside a `//` comment → **REMOVED 2026-06-27**; `_SO/Action/ActionIconConfig.asset` + `.meta` also **REMOVED 2026-06-27** (scratch-scene-only ref, not a live dependency). **KEPT — still have live components in `Vertical Slice.unity`:** `CategoryButton.cs` (GUID `497e8f581e3a959499a9fae5722d574b`) — 4 live `m_Script` components on category-button GameObjects in Vertical Slice + 4 more in `casdasda.unity`/`0.unity`; `DPadLayoutGroup.cs` (GUID `96e299f9a60070348a957cd1a8467fc2`) — 2 live `m_Script` components in Vertical Slice + 1 in each scratch scene. Human must remove those GameObjects/components first before these two can be deleted. ⚠️ **Human wiring required before the bar renders:** add `ActionCardView` to the action-card prefab; add the 4 view components to their GameObjects and re-home the moved serialized refs (tabs, strip content/prefab, estimate texts, lock modal); remove the missing-script on the `ActionUIManager` GameObject in the 3 scenes.
...
| **ActionBarUI** (controller) | `_GAME/Action/ActionBarUI.cs` | **~325** | ACTIVE · **primary action flow** · `Habitales.UI.Actions` | **Controller only** after the 2026-06-27 decomposition (see callout above). Owns the arm/disarm state machine + cross-system coordination (`ActionManager.Instance`, `[SerializeField] TileSelector`) + the frozen onboarding surface. Delegates ALL rendering to the 4 views via `[SerializeField]` refs. `OnPlantSpawnedTween` cube stub **deleted**. No pause-gate of its own (gate is in ActionManager). |
| **Action UI views** (added 2026-06-27) | `_GAME/Action/`: `ActionCardView.cs`, `ActionCategoryBar.cs`, `ActionStripView.cs`, `ActionEstimatePanel.cs`, `LockModalView.cs` | — | NEW · ACTIVE · `Habitales.UI.Actions` | Passive views driven by `ActionBarUI`. **ActionCardView** — per-prefab, serialized icon/label/boogle refs, `Bind(...)` (killed the old `transform.Find` lookups). **ActionCategoryBar** — 4 tabs → `OnCategorySelected`; `GetTabRect`. **ActionStripView** — builds cards via ActionCardView, owns Boogle wiring + highlight; `Render`/`SetVisible`/`Highlight`/`GetCardRect`; `OnActionCardClicked`/`OnLockClicked`. **ActionEstimatePanel** — tile/days/fatigue + Confirm; `Render`/`SetVisible`/`SetConfirmInteractable`/`GetConfirmRect`; `OnConfirmClicked`. **LockModalView** — `Show`/`Hide` (⚠️ place on a persistent GO with `root`→the modal child, not on the modal root itself, so its Continue-button wiring isn't gated by visibility). |
| **ActionUI** | ~~`_GAME/Action/ActionUI.cs`~~ | — | **REMOVED** (2026-06-27) | The old tile-first docked-card flow (907 LOC), superseded by `ActionBarUI`. Deleted with its `.meta` (Open Question #2 resolved). `InspectModeManager` was decoupled first (now self-contained). Listed so a stale reference doesn't resurrect it. |
| **Action UI support** | `DPadLayoutGroup.cs` (338, ns `UnityEngine.UI`, custom LayoutGroup), `CategoryButton.cs` (53), `ActionCategory.cs` (52, enum+ext), `TrashVisualConfig.cs` (8, SO) (ActionIconConfig/ActionPanelState REMOVED 2026-06-27 — orphaned by ActionUI deletion; CategoryButton KEPT — still live in scene) | — | MIXED | `DPadLayoutGroup` is a **live component** in `Scenes/Vertical Slice.unity` (2 instances) — KEEP until those GameObjects are removed. `CategoryButton` has **live components** in `Vertical Slice.unity` (4 instances) — KEEP until those category-button GameObjects are removed. `TrashVisualConfig` holds bio/nonBio sprite-variant lists; referenced by `InspectTrashAction` (see bug below). `HoverTooltip.cs` (was also an ActionUI support script) is **REMOVED (2026-06-27).** `ActionIconConfig.cs` + its `.asset` (`_SO/Action/ActionIconConfig.asset`) **REMOVED 2026-06-27**: script GUID absent from all assets (`.asset` was only in scratch scenes `casdasda.unity`/`0.unity`, not Vertical Slice); code reference in `PlayerAction.cs` line 28 is a `//` comment only. `ActionPanelState.cs` **REMOVED 2026-06-27**: zero GUID hits in any asset, zero code references outside its own definition. |
...
> - **`ActionEffect.cs`** (`ActionEffect` + `ActionEffectType` enum) — SUPPORT · one authorable per-tile effect. `PlaceEntity` (spawns a `TileEntitySO` by id, only if tile empty) or `CustomBehavior` (delegates to an `ActionEffectHook`). Only the chosen kind's field is shown — see the drawer below. ⚠️ **Must be a `[Serializable] struct`, not a class** — Artifice's reorderable list view throws `ArgumentOutOfRange` in `SwapChildren` when dragging class-typed elements; all working lists in the project (StatChange/StatCondition) are structs.
> - **`Editor/ActionEffectDrawer.cs`** — NEW (added 2026-06-25) · `[CustomPropertyDrawer(typeof(ActionEffect))]` (UIElements `CreatePropertyGUI`). Shows `entityToPlace` for **PlaceEntity** / `customBehaviour` for **CustomBehavior** and hides the other. **Replaces the original `[EnableIf]` conditionals**, which were removed: `EnableIf` hides correctly on top-level SO fields (TileEntitySO) but not on a field nested inside a struct that is a reorderable-list element, so both slots stayed visible. Artifice picks the drawer up via its `HasCustomDrawer`/`CreatePropertyGUI` path, so the look is unchanged.
...
| `ActionUI` (907) | ✅ **DELETED 2026-06-27** — ActionBarUI decomposed into controller + 4 views (`Habitales.UI.Actions`); InspectModeManager decoupled. |
...
2. ✅ **RESOLVED 2026-06-27** — committed to `ActionBarUI`; legacy `ActionUI` deleted and `ActionBarUI` decomposed into a controller + 4 passive views under `Habitales.UI.Actions` (see §G callout + `ACTIONUI_REFACTOR_PLAN.md`).

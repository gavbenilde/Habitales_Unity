...
| **Live actions (6 registered)** | `FireSuppressionAction.cs` (34), `PlantTreesAction.cs` (30), `ApplyFertilizerAction.cs` (36), `ClearTrashAction.cs` (28), `StumpDeadTreeRemovalAction.cs` (29), `InspectTrashAction.cs` (48, ns `Habitales.Entities`) | — | ACTIVE | All six registered in `ActionManager.RegisterActions` (lines 53–58). FireSuppression/ClearTrash/Stump use `AnyTileHasEntityId` + `RemoveEntity`; PlantTrees uses `SpawnById("tree_seedling")` + factory `entityId` check. ⚠️ **`ApplyFertilizerAction` writes `tile.stats.*` DIRECTLY (lines 27–34) — a LIVE Law-1 violation** (should route through `ModifyTileStats`). ⚠️ `InspectTrashAction.visualConfig` is a `[SerializeField]` on a `new`-constructed plain class → **always null at runtime** (sprite-swap silently disabled; `isInspected` now lives in `GenericTileEntity.hookState`). |
| **Dormant actions (3)** | `CreateFirebreakAction.cs` (40), `AnalyzeSoilSampleAction.cs` (44), `EcologicalSurveyAction.cs` (26) | — | DORMANT | Commented out of `RegisterActions` (lines 61–63). ⚠️ `CreateFirebreakAction` writes `tile.tv` + `tile.stats.vegetationCover` **directly** (Law-1 fix needed before reactivation). `AnalyzeSoilSample`/`EcologicalSurvey` write `tile.isAnalyzed`/`issuesRevealed` directly. |
...
| System | Source | LOC | Status | Role & notes |
|---|---|---|---|---|
| **DialogueManager** | `_GAME/Dialogue/_API/DialogueManager.cs` | **541** | ACTIVE (simplified) · singleton · `[-100]` | `Habitales.Dialogue`. **40%/day** flavor (`MESSAGE_ROLL_CHANCE = 0.40f`) + 25-day per-worker cooldown; 8 hardcoded `s_flavorLines`. Azi Tier 1/2 hardcoded inline (`AppendAziMessage`, no SO). ⚠️ **There is no "Tier 3" logic in this file** — it's purely caller-driven (RunEndCoordinator's Azi line is pushed in from outside). `RunBirthdayCheck` runs **every day** and *would* fire `birthday_group_shoutout` + 🎂 indicator if any worker had `birthdayDay` authored (live code, dormant only on data). Writes `EventContext.SetOverride`. Subscribes `ResourceManager.OnTimeAdvanced` + `RegionManager.OnRegionGenerated`. |
| **Chat App UI** | `Dialogue/App/ChatAppUI.cs` (253), `ChatBubbleUI.cs` (55), `TabRowUI.cs` (54), `StickerButtonUI.cs` (31) | — | ACTIVE | **Namespace `Habitales.UI`** (not global). `ChatAppUI` subscribes `DialogueManager.OnMessagesUpdated`/`OnUnreadChanged`; tab filter shows only tabs with ≥1 message body. |
| **Dialogue support** | `DialogueRuntimeTypes.cs` (106), `DialogueRegistry.cs` (58, SO), `DialogueTypes.cs` (44), `DialogueTabSO.cs` (16) | — | SUPPORT / LIVE | `Habitales.Dialogue`. Runtime entry/preview POCOs + the id→SO registry (live). |
| **Dormant dialogue** | `StickerSO.cs` (17), `StickerLibrary.cs` (56), `WorkerMessageTemplateSO.cs` (12), `DialogueThreadSO.cs` (25) | — | DORMANT | Sticker system + template randomizer inert (`HandleStickerSent`/`AppendWorkerThread` have no live callers; `registry.workerTemplates` empty). `DialogueThreadSO` authoring path dormant — Azi uses an inline factory, not SOs. `WorkerPortraitPool.cs` (33) is **ns `Habitales`** (global) and IS live — used by `WorkerFactory`. |
...
| System | Source | LOC | Status | Role & notes |
|---|---|---|---|---|
| **StatTypes** | `Scripts/Core/StatTypes.cs` | **52** | NEW · live | `Habitales.Core`. Enums `TargetStat` (8 stats + derived `SoilComposite`), `Comparator`, `OutcomeKind`; structs `StatChange` (stat+delta, applied directly clamped 0–100, **not** ÷6) + `StatCondition` (stat+comparator+threshold+outcome+transformTarget+`onSatisfied`). |
...
| **TileEntitySO** | `Scripts/Entities/TileEntitySO.cs` | **61** | NEW · live · `[CreateAssetMenu]` | `Habitales.Entities`. Identity + data: `entityId` (save key), `displayName`, `category`, `tileSprite`, `vfxKey`, `billboardScale`, `nextStage`, `promoteAfterDays`, `requirePromoteCondition`+`promoteWhen`, `transitionEffects`, `dailyEffects`, `deathConditions`, optional `behaviour`. ⚠️ **No `OnValidate`** (see §N). |
| **GenericTileEntity** | `Scripts/Entities/GenericTileEntity.cs` | **150** | NEW · live | Extends `TileEntity`. `OnDailyUpdate`: apply `dailyEffects` → check `deathConditions` → timed transition (`nextStage`) → optional hook. Per-instance state (`daysExisting`, `hookState` dict incl. `inspected`/burn state) lives **here**, never on the SO. |

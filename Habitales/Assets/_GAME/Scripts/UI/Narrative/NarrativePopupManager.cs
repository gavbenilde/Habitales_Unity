...
namespace Habitales.UI
{
    ...
    ///
    /// Threaded calls accept either a DialogueThreadSO (authored once, also renders in chat)
    ...
    public class NarrativePopupManager : MonoBehaviour
    {
        ...
        /// <summary>Play an authored thread (also chat-renderable) in the given style.</summary>
        public void PlayThread(DialogueThreadSO thread, in PopupStyle style, Action onComplete = null)
        {
            if (thread == null)
            {
                Debug.LogWarning($"{name}: PlayThread called with a null thread.", this);
                onComplete?.Invoke();
                return;
            }
            ...
            if (dm == null)
            {
                Debug.LogError($"{name}: DialogueManager.Instance is null — cannot resolve thread '{thread.threadID}'.", this);
                onComplete?.Invoke();
                return;
            }
            PlayLines(dm.ResolveThreadLines(thread), style, onComplete);
        }

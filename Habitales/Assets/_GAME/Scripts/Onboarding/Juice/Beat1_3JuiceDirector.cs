...
namespace Habitales.Onboarding
{
    ...
    public class Beat1_3JuiceDirector : MonoBehaviour
    {
        ...
        void HandleEntitySpawned(Tile tile, string entityId)
        {
            ...
            // TODO(juice): re-enable once the delta-tip and score-bubble track the tile's
            // world position per-frame (or live on a world-space canvas). Right now they are
            // placed once via WorldToScreenPoint, so they drift off the grid when the camera
            // moves. Disabled for now per design review.
            // StartCoroutine(FireChain(tile, gap));
            _ = gap; // keep the variable referenced while the chain is disabled
        }
        ...
        IEnumerator FireChain(Tile tile, float gap)
        {
            ...
            Vector3 worldPos = TileManager.Instance != null
                ? TileManager.Instance.GridToWorldPosition(tile.gridPosition) + deltaTipOffset
                : Vector3.zero;
            SpawnDeltaTip(worldPos);

            yield return new WaitForSeconds(gap);

            // ── Signal 3: score bubble arc to corner ──────────────────────────
            LaunchScoreBubble(worldPos);
        }
        ...
        void FlashTileColor(Tile tile)
        {
            ...
            LeanTween.value(tileGO, 0f, 1f, halfDur)
                .setOnUpdate((float t) =>
                {
                    ...
                    mr.material.SetColor("_BaseColor", Color.Lerp(originalColor, commitFlashColor, t));
                })
                .setOnComplete(() =>
                {
        ...
        void SpawnDeltaTip(Vector3 worldPos)
        {
            if (deltaTipPrefab == null) return;

            GameObject tip = Instantiate(deltaTipPrefab);
            tip.transform.position = worldPos;

            // If the tip has a DuskBubble, reuse its float-and-fade logic.
            DuskBubble bubble = tip.GetComponent<DuskBubble>();
            if (bubble != null)
            {
                bubble.PlayOneShot(worldPos, deltaTipDuration);
                return;
            }

            // Fallback: simple LeanTween float-up + alpha fade via CanvasGroup.
            CanvasGroup cg = tip.GetComponent<CanvasGroup>();
            if (cg == null) cg = tip.AddComponent<CanvasGroup>();

            LeanTween.moveY(tip, worldPos.y + 0.8f, deltaTipDuration)
                .setEaseOutQuad();
            LeanTween.alphaCanvas(cg, 0f, deltaTipDuration * 0.4f)
                .setDelay(deltaTipDuration * 0.6f)
                .setOnComplete(() => Destroy(tip));
        }

        void LaunchScoreBubble(Vector3 fromWorldPos)
        {

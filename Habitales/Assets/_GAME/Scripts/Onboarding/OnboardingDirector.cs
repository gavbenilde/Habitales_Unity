...
namespace Habitales.Onboarding
{
    ...
    public class OnboardingDirector : MonoBehaviour
    {
        ...
        private bool _actionSelectedThisBeat   = false; // polled — no OnActionSelected event exists
        private bool _seedClickedThisBeat      = false; // event: TileSelector.OnTileSelected (click phase)
        private bool _confirmedThisBeat        = false; // event: TileSelector.OnMultiSelectionConfirmed
        ...
        // Beat 3.3 (new tool) — no runtime signal exists yet; auto-advance after first day post-3.1.
        ...
        void Start()
        {
            ...
            if (RunManager.Instance != null)
                ...
                Debug.LogError($"{name}: RunManager.Instance is null — region-unlock beat will not trigger. Ensure RunManager is in the scene.", this);

            // TileSelector has no static Instance — FindObjectOfType once and cache (documented).
            _cachedSelector = FindObjectOfType<TileSelector>();
            if (_cachedSelector != null)
            {
                ...
                Debug.LogError($"{name}: No TileSelector found — confirm and tile-inspector detection will not work.", this);
        ...
        void Update()
        {
            ...
            switch (beat)
            {
                ...
                case OnboardingBeatId.Beat_Drag_Arm:
                    // POLLED: no OnActionArmed event exists. Advance once Plant Trees is armed.
                    if (!_actionSelectedThisBeat && actionBarUI != null)
                    {
                        if (actionBarUI.CurrentArmedAction is PlantTreesAction)
                        {
                ...
                case OnboardingBeatId.Beat_Drag_Confirm:
                    // Event-driven (HandleSelectionConfirmed sets _confirmedThisBeat on Confirm press).
        ...
        void EnterBeat(OnboardingBeatId id)
        {
            ...
            // The drag ghost is NOT shown on entry — it's driven by arm state (see
            // RefreshDragGhostByArmState): it appears when an action is armed, hides when disarmed.
        ...
        {
            // Fired by TileSelector.ConfirmSelection() (Confirm button). Satisfies the confirm beats.
        ...
        CoachMarkRequest BuildCoachMarkRequest(BeatDef def)
        {
            ...
            switch (def.id)
            {
                ...
                case OnboardingBeatId.Beat_Click_Place:   // GhostMouseClick
                case OnboardingBeatId.Beat_Drag_Select:   // GhostMouseDrag (arm-gated; not built here)
...
// =============================================================================
// SIGNALS THAT DON'T EXIST (had to poll instead — document for future work):
//
// 1. OnActionSelected / OnActionArmed — ActionBarUI.ArmAction() has no event.
//    Poll: ActionBarUI.ArmedAction static getter (added as a companion edit below).
//    Alternatively: add `public static event Action<PlayerAction> OnActionArmed` to ActionBarUI.
//
// 2. OnActionConfirmed — no event fires when the player presses the Confirm button.
//    Poll: ActionManager.IsActionRunning transitions false → true.
//
// 3. OnNewToolUnlocked — beat 3.3 (new tool unlocked) has no runtime signal.
//    Assumption: auto-advance after the first day following beat 3.1.
//
// 4. TileSelector.Instance — TileSelector has no static singleton.
//    Workaround: FindObjectOfType<TileSelector>() on Start + cache.
//
// =============================================================================

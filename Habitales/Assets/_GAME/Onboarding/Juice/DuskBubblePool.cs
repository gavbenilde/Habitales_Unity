...
{
    /// <summary>
    /// Owns the dusk-bubble object pool and the Law-2 meaning-event hooks. Subscribes to
    /// <c>TileManager.OnEntitySpawned</c> (plant placed) and <c>TileManager.OnEntityEvolved</c>
    /// (stage promotion: seedling→sapling→mature) and pops a pooled <see cref="DuskBubble"/>
    /// at the tile's world position. Event-driven only — never per-tile-per-day. The pool size
    /// is the hard cap on concurrent bubbles, so a region-generation burst can never spawn
    ...
    public class DuskBubblePool : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            if (TileManager.Instance != null)
            {
                ...
                Debug.LogWarning($"{name}: TileManager.Instance is null at Start — dusk bubbles won't fire on spawns or evolves.", this);
        }
